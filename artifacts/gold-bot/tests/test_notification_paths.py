...
class NotificationPathTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_auto_alert_analysis_uses_strict_consensus_path(self):
        ...
        with patch.object(
            ...
            result = await alerts._safe_analyze("M15", mode="scalp")

        self.assertIs(result, analysis_result)
    ...
    async def test_setup_forming_alert_is_sent_on_live_scan(self):
        ...
        self.assertIn("PROVISIONAL EARLY ENTRY", text)
        self.assertIn("not saved as an active trade", text)
    ...
    async def test_combined_scan_keeps_setup_and_momentum_alerts_independent(self):
        ...
        self.assertTrue(any("PROVISIONAL EARLY ENTRY" in text and "SCALP  XAU/USD  M15" in text for text in texts))
