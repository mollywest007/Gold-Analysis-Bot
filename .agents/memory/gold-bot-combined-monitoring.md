...
**How to apply:** Namespace locks and active-trade ownership by stream only when combined mode is active. Prefix every combined-mode entry, setup, momentum, result, cooldown, and reminder notification with `SCALP ALERT` or `INTRA-HOUR ALERT`; keep `interval` only as the internal persistence key.